...
const Feedback = ({ profile }) => {
  ...
  // ✅ Pull parentId from profile
  const parentId = profile?.parents?.[0]?.id || null;
  ...
  const fetchFeedback = useCallback(async () => {
    if (!token) return;
    const parentAdminId = profile?.parentAdminId || profile?.parents?.[0]?.id;
    if (!parentAdminId) return;

    try {
      const response = await fetch(`${API_BASE_URL}/api/admin/feedback/parent/${parentAdminId}`, {
  ...
  // Reset category add state when form closes
  useEffect(() => {
    if (!openForm) {
  ...
  const handleSubmit = async () => {
    ...
    // ✅ Use first classScheduleId for payload
    const classScheduleId = classScheduleIds[0];

    const parentAdminId = profile?.parentAdminId || profile?.parents?.[0]?.id;
    const payload = {
      bookingId,
      parentAdminId,           // ✅ parentAdminId from profile.parents[0].id
      // ✅ parentId from profile.parents[0].id
      classScheduleId,
      serviceType: DISPLAY_SERVICE_TYPE,
      feedbackType,
      category,
      notes,
      agentAssigned: agentIds,  // ✅ array
    };
    ...
    try {
      ...
      setOpenForm(false);
      setFormData({ classScheduleIds: [], agentIds: [], feedbackType: "", category: "", notes: "" });
      fetchFeedback();
    } catch (err) {
      showError("Error", err.message);
    }
  ...
  // ✅ Prefill selected agents from assignedAgents array when resolve view opens
  useEffect(() => {
  ...
  // Helper: get agents array from a feedback row (supports both field names)
  const getAgents = (row) => {
    ...
    <>
      ...
      <div className={`pt-1 bg-gray-50 min-h-screen md:px-4 ${openResolve ? 'hidden' : 'block'}`}>
        ...
        <button
          ...
          className="bg-[#237FEA] md:absolute right-5 top-3 flex items-center gap-2 cursor-pointer text-white px-4 py-2 rounded-xl hover:bg-blue-700 text-sm md:text-base font-semibold"
        >
                          ...
                          <tr key={idx} className="border-t font-semibold text-[#282829] border-[#EFEEF2] hover:bg-gray-50">
                            ...
                            <td className="p-4">
                              <div className="flex items-center gap-3">
                                ...
                                {/* ✅ Only show Resolve when status is "not_resolved" or "not_resolve" */}
          ...
          <div className="fixed inset-0 bg-[#00000047] bg-opacity-40 flex items-center justify-center z-50">
            <div className="bg-white rounded-2xl w-[95%] md:w-[420px] md:max-h-[90vh] overflow-auto shadow-lg relative">
              ...
              <div className="p-6 space-y-4">
                ...
                {/* ✅ All students' classes — each prefilled and disabled */}
                <div>
