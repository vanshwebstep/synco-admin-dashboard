...
const Coach = () => {
    ...
    const {
        ...
        venues,
        statsRecruitment,
        createCoachRecruitment,
        sendCoachMail,
    } = useRecruitmentTemplate() || {};
    ...
    const handleExport = () => {
        ...
        XLSX.writeFile(workbook, "coaches_export.xlsx");
    };

    const venueOptions = venues?.map((v) => ({
        value: v.id,
        label: `${v.name} ${v.area}`.trim() || v.email,
    }));
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.availableVenueWork || formData.availableVenueWork.length === 0) { formErrors.availableVenueWork = "At least one venue must be selected"; }
    ...
    const handleSendEmail = () => {
        if (selectedIds && selectedIds.length > 0) {
            const filteredCoaches = currentData.filter((c) => selectedIds.includes(c.id));
            const emails = filteredCoaches.map((c) => c.email).filter(Boolean);
    ...
    const handleAssignAgent = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (fetchRecruitment) fetchRecruitment(); // Refresh data
            } else {
    ...
    const currentData = useMemo(
        () => filteredRecruitment.slice(startIndex, startIndex + rowsPerPage),
        [filteredRecruitment, startIndex, rowsPerPage]
    );
                        ...
                        <tbody>
                            {currentData.length === 0 ? (
                                ...
                                currentData.map((coach) => {
                                    ...
                                    const experience = coach?.candidateProfile?.footballExperience || "-";
                                ...
                                if (startPage > 1) {
                                    pageButtons.push(
                                        <button key={1} onClick={() => setCurrentPage(1)}
                                            className={`px-3 py-1 rounded-md border ${currentPage === 1 ? "bg-blue-500 text-white border-blue-500" : "hover:bg-gray-100 border-gray-300"}`}>
                                            1
                                        </button>
                                    );
                                ...
                                for (let i = startPage; i <= endPage; i++) {
                                    pageButtons.push(
                                        <button key={i} onClick={() => setCurrentPage(i)}
                                            className={`px-3 py-1 rounded-md border ${currentPage === i ? "bg-blue-500 text-white border-blue-500" : "hover:bg-gray-100 border-gray-300"}`}>
                                            {i}
                                        </button>
                                    );
                                }
                                ...
                                if (endPage < totalPages) {
                                    ...
                                    pageButtons.push(
                                        <button key={totalPages} onClick={() => setCurrentPage(totalPages)}
                                            className={`px-3 py-1 rounded-md border ${currentPage === totalPages ? "bg-blue-500 text-white border-blue-500" : "hover:bg-gray-100 border-gray-300"}`}>
                                            {totalPages}
                                        </button>
                                    );
                                }
                ...
                <div className="fixed inset-0 bg-[#0000008f] bg-opacity-40 flex items-center justify-center z-50">
                    <div className="bg-white w-full max-w-lg rounded-2xl shadow-xl p-6">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4 max-h-[75vh] overflow-y-auto pr-2">
                            ...
                            {/* Postcode */}
                            <div className="flex flex-col gap-1">
                                <label className="text-sm text-gray-500">London Postcode</label>
                                <input
                                    ref={postcodeRef}
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="text-sm text-gray-500">Do you have access to your own vehicle?</label>
                                <div className="flex gap-6">
                                    {[{ label: "Yes", value: "true" }, { label: "No", value: "false" }].map(({ label, value }) => (
                                        <label key={value} className="flex items-center gap-2 cursor-pointer text-sm text-[#282829]">
                                            <input
                                                type="radio"
                            ...
                            {/* Qualifications */}
                            <div className="flex flex-col gap-2">
                                <label className="text-sm text-gray-500">Please select which qualifications you have</label>
                                <div className="flex flex-col gap-2">
                            ...
                            {/* Football Experience */}
                            <div className="flex flex-col gap-1">
                                <label className="text-sm text-gray-500">How many years football coaching experience do you have?</label>
                                <Select
                            ...
                            {/* Available Venues */}
                            <div className="flex flex-col gap-1">
                                <label className="text-sm text-gray-500">Please select which venues you are available for work</label>
                                <Select
                                    isMulti
                                    options={venueOptions}
                                    styles={selectStyles}
                                    placeholder="Choose Venue(s)"
                                    value={venueOptions?.filter((o) => formData.availableVenueWork.includes(o.value))}
                                    onChange={(selected) => {
                            ...
                            {/* Cover Note */}
                            <div className="flex flex-col gap-1">
                                <label className="text-sm text-gray-500">Please Add a short cover note (500 words max)</label>
                                <textarea
                                    ref={coverNoteRef}
                                    placeholder="Message"
                                    rows={4}
                            ...
                            {/* How did you hear */}
                            <div className="flex flex-col gap-2">
                                <label className="text-sm text-gray-500">How did you hear about this opportunity?</label>
                                <div className="flex flex-col gap-2">
                            ...
                            <div className="flex justify-end gap-3 pt-2 pb-1">
                                ...
                                <button
                                    ...
                                    className={`px-5 py-2 rounded-lg bg-[#237FEA] text-white text-sm font-medium hover:bg-blue-700 transition ${isSubmitting ? "opacity-60 cursor-not-allowed" : ""}`}
                                >
            ...
            {showFilter && (
                <div className="md:w-4/12 md:mt-0 p-4 mt-4 text-base space-y-5">
                    <div className="mb-4 bg-white rounded-2xl p-4">
                        <h3 className="font-semibold text-black text-[24px] mb-4">Search now</h3>
                    ...
                    <div className="bg-white p-4 rounded-xl">
                        <div className="flex justify-between items-center mb-2">
                            ...
                            >
                                <img src="/reportsIcons/filter.png" className="w-4" alt="" />
                                Apply Filter
                            </button>
                        </div>
                        ...
                        <div className="rounded p-4 mt-6 text-center text-base w-full max-w-md mx-auto">
                            ...
                            <div className="grid grid-cols-7 text-xs gap-1 text-[18px] text-gray-500 mb-1">
                                {["M", "T", "W", "T", "F", "S", "S"].map((day, indx) => (
                                    <div key={indx} className="font-medium text-center">{day}</div>
                                ))}
                            </div>
                    ...
                    <div className="grid blockButton md:grid-cols-3 gap-3 mt-4">
                        ...
                        <button onClick={handleExport} className="flex items-center justify-center gap-1 bg-[#237FEA] text-white text-sm py-3 rounded-lg hover:bg-blue-700 transition">
