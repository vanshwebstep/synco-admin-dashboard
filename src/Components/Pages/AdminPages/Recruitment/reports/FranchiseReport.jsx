...
export default function FranchiseReport() {
  ...
  const { fetchFranchiseReport, franchiseReport } = useRecruitmentTemplate() || {};

  useEffect(() => {
    const loadData = async () => {
      if (!fetchFranchiseReport) return;
      setLoading(true);
      try {
        await fetchFranchiseReport();
      } catch (e) {
        // swallow - UI will use available franchiseReport if any
        console.error("fetchFranchiseReport failed", e);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleChange = (selectedOption) => {
    ...
    fetchFranchiseReport(value); // send only when value exists
  };
  // safe getters and fallbacks from franchiseReport
  console.log('franchiseReport', franchiseReport)
  const api = franchiseReport || {}; // entire response object expected to be what your hook exposes
  const data = franchiseReport || {};
  const report = data?.report || {};
  const chartDataFromApi = data?.chartData || {};
  const demographics = data?.franchise_demographics || {};
  const qualificationsApi = data?.qualificationsAndExperince || {};
  ...
  const topAgentsApi = data?.topAgentsMostHires || [];
  const callStats = {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: `${report?.discoveryCalls?.current ?? 0}`,
      diff: `${report?.discoveryCalls?.conversionRatee ?? 0}`,
      sub: "vs. prev period",
      subvalue: `${report?.discoveryCalls?.previous ?? 0}`,
    },
    {
  ...
  const qualifications = useMemo(() => {
    ...
    return [
      { label: "FA Level 1 Qualification", value: qualificationsApi?.faQualification ?? 0, img: '/reportsIcons/fa.png' },
      { label: "FA Level 2 Qualification", value: qualificationsApi?.coachingExperience ?? 0, img: '/reportsIcons/label2.png' },
      { label: "Past Business Ownership", value: qualificationsApi?.noExperience ?? 0, img: '/reportsIcons/business.png' },
      { label: "4-5 Years of Coaching Experience", value: qualificationsApi?.dbsCertificate ?? 0, img: '/reportsIcons/mainuser.png' },
    ];
  }, [qualificationsApi]);
  ...
  const onboardingResults = useMemo(() => {
    return [
      { label: "Average Questionnaire Scorecard", value: onboardingApi?.averageCallGrade ?? "0%" },
      { label: "Average Discovery Call Assessment Grade", value: onboardingApi?.averagePracticalAssessmentGrade ?? "0%" },
      { label: "Average Practical Assessment Grade", value: onboardingApi?.averageCoachEducationPassMark ?? "0%" },
    ];
  }, [onboardingApi]);
  ...
  console.log('topAgents0', topAgents)
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(workbook, "franchise-report.xlsx");
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white p-5 rounded-2xl">
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-[22px] font-semibold text-gray-800">Franchisee Demographics</h3>
                <EllipsisVertical className="text-gray-400" />
              </div>
