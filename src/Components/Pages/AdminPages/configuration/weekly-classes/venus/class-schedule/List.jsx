...
const List = () => {
    ...
    const scrollToStartTime = () => {
        ...
        requestAnimationFrame(() => {
            const list = document.querySelector(".react-datepicker__time-list");
            if (!list) return;
            const normalizedTime = formData.startTime.trim();
            const target = Array.from(list.children).find((el) =>
    ...
    // ✅ usedLevels & levelOptions placed AFTER formData and isEditing
    const usedLevels = filteredSchedules
        .filter((s) => !isEditing || s.id !== formData.id)
        .map((s) => s.level);
    ...
    const levelOptions = [
        ...
        ...opt,
        isDisabled: usedLevels.includes(opt.value),
        label: usedLevels.includes(opt.value) ? `${opt.label} (already added)` : opt.label,
    }));
    ...
    // ✅ Inline validation — no toast/swal
    const validate = () => {
        const newErrors = {};
        ...
        if (formData.startTime && formData.endTime) {
            ...
            if (startMinutes === endMinutes)
                newErrors.endTime = "Start and end time cannot be the same";
    ...
    const inputClass = "w-full h-[48px] border rounded-xl px-3 text-sm";
    ...
    const errorSelectStyles = (hasError) => ({
        control: (provided) => ({
            ...provided,
            minHeight: "48px",
            height: "48px",
            borderRadius: "12px",
            borderColor: hasError ? "#ef4444" : "#E2E1E5",
            boxShadow: "none",
            "&:hover": { borderColor: hasError ? "#ef4444" : "#E2E1E5" },
        }),
    });

    const canCreate = checkPermission({ module: 'class-schedule', action: 'create' });
    const canUpdate = checkPermission({ module: 'class-schedule', action: 'update' });
    const canDelete = checkPermission({ module: 'class-schedule', action: 'delete' });
    const cancelSession = checkPermission({ module: 'cancel-session', action: 'view-listing' });
        ...
        <div className="pt-1 bg-gray-50 min-h-screen">
            <div className={`md:flex pe-4 justify-between items-center mb-4 w-full`}>
                <h2
                    ...
                    className="md:text-[28px] cursor-pointer hover:opacity-80 font-semibold mb-4 flex gap-2 items-center p-5"
                >
                    <img src="/members/Arrow - Left.png" className="w-6" alt="" /> Edit Class Schedule
                </h2>
                {canCreate && (
                    <button
                        ...
                        <div className="bg-white rounded-3xl p-6 shadow-xl">
                            ...
                            {filteredSchedules.map((item, index) => (
                                <React.Fragment key={index}>
                                    <div
                                        className={`flex flex-col md:flex-row justify-between items-center border ${item.highlight ? "border-red-400" : "border-gray-200"} rounded-xl px-4 pr-16 py-4 mb-3 hover:shadow transition`}
                                    >
                                        ...
                                        <div className="flex items-center mt-4 md:mt-0 gap-4">
                                            ...
                                            <button
                                                ...
                                                className="ml-4 flex font-semibold items-center gap-2 whitespace-nowrap px-4 pr-6 py-2 border rounded-xl text-[16px] font-medium text-[#237FEA] border-blue-500 hover:bg-blue-50"
                                            >
                                                {openClassIndex === index ? 'Hide sessions' : 'View sessions'}
                                                <img src="/images/icons/bluearrowup.png" className={`${openClassIndex === index ? '' : 'rotate-180'} transition-transform`} alt="" />
                                            </button>
                                        </div>
                                    </div>
                                                                ...
                                                                <div key={term.id}>
                                                                    ...
                                                                    >
                                                                        <div className='flex mb-4 items-center gap-8 justify-start'>
                                                                            <div>
                                                                                <img src="/images/icons/blackarrowup.png" className={`${openTerms[term.id] ? "" : "rotate-180"} transition-transform`} alt="" />
                                                                            </div>
                                                                            <div>
                                                                                <p className="font-semibold text-[16px]">{term.termName}</p>
                                                                    ...
                                                                    <div className={`transition-all duration-300 overflow-hidden ${openTerms[term.id] ? "max-h-[1000px]" : "max-h-0"}`}>
                                                                        {(() => {
                                                                            let sessions = [];
                                                                            if (typeof term.sessionsMap === "string") {
                                                                                try { sessions = JSON.parse(term.sessionsMap); }
                                                                                catch (err) { sessions = []; }
                    ...
                    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                        <div className="bg-white rounded-3xl w-[90%] md:w-[900px] p-6 relative shadow-xl">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <button
                                    type="button"
                                    onClick={() => { setOpenForm(false); setErrors({}); }}
                                ...
                                <div className="flex gap-4">
                                    ...
                                    <div className='block md:w-1/3'>
                                        ...
                                        <input
                                            ...
                                            className={`${inputClass} ${errors.className ? 'border-red-500' : 'border-[#E2E1E5]'}`}
                                        />
                                        {errors.className && (
                                    ...
                                    <div className='block md:w-1/3'>
                                        ...
                                        <Select
                                            ...
                                            styles={errorSelectStyles(!!errors.level)}
                                    ...
                                    <div className='block md:w-1/3'>
                                        ...
                                        <input
                                            ...
                                            className={`${inputClass} ${errors.capacity ? 'border-red-500' : 'border-[#E2E1E5]'}`}
                                        />
                                        {errors.capacity && (
                                ...
                                <div className="flex gap-4">
                                    ...
                                    <div className='w-1/2'>
                                        ...
                                        <Select
                                            ...
                                            options={dayOptions}
                                            isClearable
                                            styles={errorSelectStyles(!!errors.day)}
                                            classNamePrefix="react-select"
                                        />
                                        {errors.day && (
                                        ...
                                        <div className="flex gap-4 w-full">
                                            ...
                                            <div className="w-1/2">
                                                ...
                                                <DatePicker
                                                    ...
                                                    className={`${inputClass} ${errors.startTime ? '!border-red-500' : 'border-[#E2E1E5]'}`}
                                                />
                                                {errors.startTime && (
                                            ...
                                            <div className="w-1/2">
                                                ...
                                                <DatePicker
                                                    ...
                                                    className={`${inputClass} ${errors.endTime ? '!border-red-500' : 'border-[#E2E1E5]'}`}
                                                />
                                                {errors.endTime && (
