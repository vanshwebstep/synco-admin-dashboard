...
const Create = () => {
    ...
    const handleMapSession = (termId) => {
        console.log("🔹 handleMapSession called");
        console.log("➡️ termId:", termId);
        console.log("➡️ isGroupSaved:", isGroupSaved);
        console.log("➡️ activeTerm:", activeTerm);
        console.log("➡️ isMapping (before):", isMapping);

        if (!isGroupSaved) {
            console.warn("⚠️ Group not saved yet");
            alert('Please save the group name first');
            return;
        }

        if (!activeTerm) {
            console.warn("⚠️ No active term selected");
        ...
        if (!isMapping) {
            console.log("🟢 Opening mapping section...");
            setIsMapping(true);

            setTimeout(() => {
                console.log("📍 Trying to scroll to map section...");
                console.log("➡️ mapSectionRef:", mapSectionRef.current);
                ...
                if (mapSectionRef.current) {
                    ...
                    console.error("❌ mapSectionRef is null");
                }
            }, 100);
        } else {
            console.log("🔴 Closing mapping section...");
            setIsMapping(false);
        }

        console.log("➡️ isMapping (after toggle expected):", !isMapping);
    };
    ...
    const handleEndDate = (termId, date) => {
        ...
        setTerms(prev =>
            prev.map(t => {
                ...
                // ✅ STEP 1: existing map (VERY IMPORTANT)
                ...
                while (current <= end) {
                    ...
                    if (
                        ...
                        // ✅ STEP 2: preserve if exists
                        if (existingMap.has(dateStr)) {
                            sessions.push(existingMap.get(dateStr));
                        } else {
                            sessions.push({
                                sessionDate: dateStr,
                                sessionPlanId: null,
    ...
    const handleSaveMappings = (termId) => {
        if (!termId) {
            console.error("❌ Missing termId in handleSaveMappings");
            return;
        }
        ...
        // ✅ SAVE DIRECTLY INTO TERM
        setTerms(prev =>
            prev.map(t =>
                t.id === termId
                    ? {
                        ...t,
                        sessionsMap: [...sessionMappings],
                        isDirty: true // 🔥 IMPORTANT
                    }
                    : t
            )
        );
    ...
    const handleSaveAll = async () => {
        ...
        try {
            ...
            const termsToSave = terms.filter(
                t => !savedTermIds?.has(t.id) || t.isDirty
            );
            console.log('termsToSave', termsToSave)
                                                ...
                                                >
                                                    <SessionPlanSelect
                                                        ...
                                                        usedSessionPlans={sessionMappings
                                                            .map((s, i) => i !== index ? s.sessionPlanId : null)
                                                            .filter(id => id)} />
